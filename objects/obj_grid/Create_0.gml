...
//	var endRail = instance_create_depth(get_x_from_pos(width-1),get_y_from_pos(floor(height/2)),5,obj_rail)
	...
	ds_map_replace(grid[width-2, floor(height/2)+1], "borderArray", get_bordering(width-2,floor(height/2)+1,grid))
	ds_map_replace(grid[width-1, floor(height/2)], "mountain1", 0)
	ds_map_replace(grid[width-1, floor(height/2)], "cost", 1)
	ds_map_replace(grid[width-1, floor(height/2)], "ore", 0)
	ds_map_replace(grid[width-1, floor(height/2)], "borderArray1", get_bordering1(width-1,floor(height/2),grid))
	ds_map_replace(grid[width-2, floor(height/2)], "borderArray1", get_bordering1(width-2,floor(height/2),grid))
	ds_map_replace(grid[width-1, floor(height/2)+1], "borderArray1", get_bordering1(width-1,floor(height/2)+1,grid))
	ds_map_replace(grid[width-1, floor(height/2)-1], "borderArray1", get_bordering1(width-1,floor(height/2)-1,grid))
	ds_map_replace(grid[width-2, floor(height/2)-1], "borderArray1", get_bordering1(width-2,floor(height/2)-1,grid))
	ds_map_replace(grid[width-2, floor(height/2)+1], "borderArray1", get_bordering1(width-2,floor(height/2)+1,grid))
