 ...
 switch (incrementer)
 {
  ...
  case 4: str = "Placing rails next to each other connects them. We need to connect the starting and ending pieces before the train comes!";
	with(obj_grid_tutorial){
		var lookatme = instance_create_depth(get_x_from_pos(0), get_y_from_pos(floor(height/2)),-100,obj_lookatme);  
		var lookatme2 = instance_create_depth(get_x_from_pos(width-1), get_y_from_pos(floor(height/2)),-100,obj_lookatme);  
	}
	break;
